import { startWorkers } from './worker-helpers.js';
...
export function encode(
	...
	try {
		...
		var ptr1 = isLikeNone(frame_durations)
			? 0
			: passArray32ToWasm0(frame_durations, wasm.__wbindgen_malloc);
		...
		wasm.encode(
			...
			width,
			height,
			isLikeNone(fps) ? 0xffffff : fps,
			ptr1,
			len1,
			isLikeNone(quality) ? 0xffffff : quality,
			!isLikeNone(repeat),
			isLikeNone(repeat) ? 0 : repeat,
			!isLikeNone(resize_width),
			isLikeNone(resize_width) ? 0 : resize_width,
			!isLikeNone(resize_height),
			isLikeNone(resize_height) ? 0 : resize_height
		);
...
function __wbg_init_memory(imports, maybe_memory) {
	imports.wbg.memory =
		maybe_memory || new WebAssembly.Memory({ initial: 18, maximum: 16384, shared: true });
}
